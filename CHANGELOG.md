...
- **`kb superseded --kb=<name> [--format=md|json] [--k=<n>] [--include-clean]`.** Read-only review command for finding obsolete, contradicted, deprecated, dormant, stale, or possibly superseded markdown notes. It scans lifecycle frontmatter first (`contradicted_by`, deprecated-like status fields, stale `last_verified_at`, and low active `confidence`), then adds conservative semantic-neighbor evidence from the existing FAISS search path when another same-KB note is newer, higher-confidence, or active. JSON output is agent-friendly; markdown output is human-readable. Closes #232.
- **`kb remember --lesson --title=<title> --stdin --yes`.** First-class agent-task-lesson template for the `kb remember` write surface. Defaults `--kb` to `agent-task-lessons` (override with `--kb=<other>`) and auto-creates that knowledge base on first use, so agents can record a generic transferable lesson with one obvious command and no KB-name guesswork. The body must contain `## Mistake`, `## Why it happened`, and `## Better next time` sections — heading text is matched case-insensitively, with trailing punctuation stripped and `Mistakes` (plural) accepted, but the **level is enforced**: `# Mistake` (H1) and `### Mistake` (H3) do NOT satisfy the requirement, so downstream anchor links and grep stay consistent. Headings inside fenced code blocks never match because the validator walks the markdown AST. Empty stdin or missing sections produce an actionable validation report (exit 2) instead of a silent low-quality note: `--format=json` (default) emits `{action: "lesson-validation", write_performed: false, lesson: true, knowledge_base_name, empty_input, missing_sections, found_sections, skeleton, decision_hint: {summary, recommended_agent_actions}}` so the agent can pipe the returned skeleton back through stdin; `--format=md` prints a header plus the same skeleton. Successful writes tag the JSON summary with `lesson: true` and `write_performed: true` and continue to honor the `#154` semantic-preflight guard. Documentation at `docs/agent-task-lessons.md` covers when to use lesson writes versus project-specific docs/skills/CLAUDE.md. Closes #200.
...
- **`kb doctor [--format=md|json]` aggregate health command.** Reports the active provider/model, active index version and mtime, per-KB modified/new stale counts, embedding backend health, CLI version, linked-checkout path when detectable, and local git HEAD vs `origin/main`. The command is read-only and exits non-zero when required health checks such as active model resolution, index presence, or backend reachability fail. Closes #180.
...
- Streamable HTTP transport behind `MCP_TRANSPORT=http`. Stdio remains the default; setting `MCP_TRANSPORT=http` plus `MCP_AUTH_TOKEN` exposes the MCP server at `POST /mcp` with stateful `Mcp-Session-Id` routing, bearer-token auth, CORS allow-list handling, session cleanup on disconnect, and graceful shutdown. Closes the streamable-http portion of #48.
