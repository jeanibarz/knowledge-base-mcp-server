...
| Per-KB write policy | `<kb>/.kb-policy.json` with `{"mutations":"allow"|"deny"}` | missing file or omitted `mutations` means `allow` | MCP `add_document` / `delete_document`; CLI `kb remember` / `kb capture` / `kb import-url` / `kb promote --yes` / `kb ask --save-transcript` managed note writes | Implemented | direct filesystem edit of the policy file only | write `{"mutations":"deny"}` to a shelf policy, then run `kb remember --kb=<name> --title=probe --stdin --yes --no-check-similar` |
