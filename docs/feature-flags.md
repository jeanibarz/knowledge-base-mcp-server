...
| Query decomposition | `kb search --mode=hybrid --decompose`, `--decompose-provider=rule\|llm`, `--decompose-max-subqueries=<n>`, `--decompose-max-iterations=<n>`, `--decompose-max-candidates=<n>`, `--decompose-timeout-ms=<n>` | off | CLI hybrid search, BEIR `hybrid+decompose` benchmark mode | Implemented, opt-in | per-call flags; `llm` provider uses `KB_DECOMPOSE_LLM_ENDPOINT` or `KB_LLM_ENDPOINT` and falls back to `rule` | `kb search "multi hop query" --mode=hybrid --decompose --format=json` |
| Query decomposition cache | `KB_DECOMPOSE_CACHE_ENABLED` | off | LLM query decomposition | Implemented, opt-in | `KB_DECOMPOSE_CACHE_LRU_MAX` (default `256`), `KB_DECOMPOSE_CACHE_DISK_MAX_BYTES` (default `67108864`) | repeat an LLM-decomposed query, then inspect `$FAISS_INDEX_PATH/cache/query-decompositions/` |
...
| Provider circuit breaker | `KB_PROVIDER_BREAKER`, `KB_PROVIDER_BREAKER_FAILURE_THRESHOLD`, `KB_PROVIDER_BREAKER_COOLDOWN_MS` | on, `3`, `30000` | Embedding and LLM provider calls | Implemented | none | Stop the provider, trigger repeated calls, then confirm later calls fail immediately until cooldown |
...
| Per-KB write policy | `<kb>/.kb-policy.json` with `{"mutations":"allow"|"deny"}` | missing file or omitted `mutations` means `allow` | MCP `add_document` / `delete_document`; CLI `kb remember` / `kb capture` managed note writes | Implemented | direct filesystem edit of the policy file only | write `{"mutations":"deny"}` to a shelf policy, then run `kb remember --kb=<name> --title=probe --stdin --yes --no-check-similar` |
...
| Allowed Host headers (DNS-rebinding) | `MCP_ALLOWED_HOSTS` | loopback aliases from bind addr/port | HTTP/SSE MCP server | Implemented | none | `MCP_TRANSPORT=http MCP_AUTH_TOKEN=<32+ chars> MCP_ALLOWED_HOSTS=kb.internal:8765 node build/index.js` |
