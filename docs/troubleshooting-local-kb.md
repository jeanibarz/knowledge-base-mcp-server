...
`kb doctor` reports active-model resolution, index path and version, stale file counts, embedding backend health, ask and relevance-gate LLM endpoint readiness, CLI package and symlink state, git drift for linked checkouts, and the latest index-update summary. Partial or failed updates, deferred warnings, or skipped files are non-OK `index_update`/`staleness` checks even when the index was freshly rewritten. Use it before deleting index files, changing client configuration, or debugging local LLM answers.

It also runs a `disk_space` check that reuses the same estimator as the `kb reindex` disk-space guard, so `kb doctor` and `kb reindex` never disagree about whether a rebuild fits on the volume. The check is `error` when free space under `${FAISS_INDEX_PATH}` is below the estimated reindex write plus the `KB_MIN_FREE_DISK_BYTES` margin — the exact condition that makes `kb reindex` refuse to start — `warn` when free space clears that requirement but by less than a 2× headroom factor, and `ok` when comfortably above. When `statfs` is unavailable it degrades to `ok` (free space reported as unknown), mirroring the guard.
...
| `kb doctor` reports `index_update` or `staleness` WARN/ERROR after a refresh | The latest index update was partial/failed, deferred files, or recorded one or more failures/skips; file mtimes alone cannot prove the rebuild was complete | `kb doctor --format=json` | Inspect `last_index_update.failures` and warnings; rerun `kb search "known phrase" --kb=<name> --refresh` after fixing the reported provider or loader failure. |
...
| `kb doctor` reports `disk_space` WARN/ERROR, or a nightly `kb reindex` silently does nothing | Free space under `${FAISS_INDEX_PATH}` is below (WARN: near) the estimated reindex write plus the `KB_MIN_FREE_DISK_BYTES` margin, so `kb reindex` refuses to start | `kb doctor --format=json` | Inspect `disk_space.available_bytes` vs `disk_space.required_bytes`; free disk space or lower `KB_MIN_FREE_DISK_BYTES`, then rerun `kb reindex`. |
