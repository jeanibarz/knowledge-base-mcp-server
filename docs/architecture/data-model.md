...
| `frontmatter` | `LiftedFrontmatter` | no | `liftFrontmatter(frontmatter, filePath)` at `src/file-ingest.ts:74-100`; malformed frontmatter falls back to `{ kb_policy: { no_llm_context: true } }` for LLM-boundary safety | visible after sanitization; `extras` hidden by default |
