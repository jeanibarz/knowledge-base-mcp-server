...
Issue [#221](https://github.com/jeanibarz/knowledge-base-mcp-server/issues/221) adds a second guard contract for chunk metadata and opt-in delimiters. By default (`KB_INJECTION_GUARD` unset or `tag`), each chunk is scanned for common indirect-prompt-injection signals and the additive `metadata.injection_signals` array is emitted with the result. `KB_INJECTION_GUARD=wrap` wraps chunk content in an `<untrusted-doc>` envelope, and `KB_INJECTION_GUARD=both` combines wrapping with metadata signals. Embedded configured opening or closing tokens are reversibly neutralized so untrusted content cannot terminate its own envelope; the configured closing token also produces a detection signal. `KB_INJECTION_GUARD=off` disables only this metadata/wrap guard. `KB_INJECTION_GUARD_BYPASS_KBS` skips shield signals, metadata detection, and wrapping for explicitly named KBs that intentionally study injection content.
