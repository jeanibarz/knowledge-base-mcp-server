...
The server applies retrieval-time content guards before chunks cross the formatter boundary. Issue [#217](https://github.com/jeanibarz/knowledge-base-mcp-server/issues/217) adds a strictly-additive, signal-only scanner (`src/kb-shield.ts`, wired through `src/formatter.ts`) that annotates each chunk with a top-level `injection_signals: Array<{rule, span_start, span_end}>` field when a versioned ruleset hits. The field is *evidence* — the chunk's `content` is not modified by the shield, and the markdown view surfaces an inline `> ⚠ injection-signal: <rule>` line so a human reviewer notices the same hit. Operators can disable the scanner with `KB_SHIELD=off` (the field is omitted entirely), and the ruleset is versioned via `KB_SHIELD_RULESET_VERSION` (currently `v2`) so additions are observable to downstream consumers. The v2 `Obfuscation.ZeroWidth`, `Obfuscation.BidiControl`, and `Obfuscation.UnicodeTag` rules flag individual invisible controls using the same classifications as the injection guard. Their half-open spans use UTF-16 code-unit offsets into the original chunk, covering one unit for zero-width/bidi controls and two for Unicode tags. These are review signals even in otherwise benign text; v1 rule IDs and matching spans remain unchanged.

Issue [#221](https://github.com/jeanibarz/knowledge-base-mcp-server/issues/221) adds a second guard contract for chunk metadata and opt-in delimiters. By default (`KB_INJECTION_GUARD` unset or `tag`), each chunk is scanned for common indirect-prompt-injection signals and the additive `metadata.injection_signals` array is emitted with the result. `KB_INJECTION_GUARD=wrap` wraps chunk content in an `<untrusted-doc>` envelope, and `KB_INJECTION_GUARD=both` combines wrapping with metadata signals. Embedded configured opening or closing tokens are reversibly neutralized so untrusted content cannot terminate its own envelope; the configured closing token also produces a detection signal. Wrapper configuration fails closed on ambiguous source templates or Unicode-multiline delimiters, source metadata line separators are escaped, and truncation accounts for the neutralized payload size before rebuilding an envelope. `KB_INJECTION_GUARD=off` disables only this metadata/wrap guard. `KB_INJECTION_GUARD_BYPASS_KBS` skips shield signals, metadata detection, and wrapping for explicitly named KBs that intentionally study injection content.
...
`MCP_TRANSPORT` defaults to `stdio` and the rest of this document assumes that baseline. When the operator opts into `sse` or `http`, the server stands up a `node:http` listener (`src/transport/sse.ts:1-22`, `src/transport/http.ts:1-38`) sharing the dispatch gates defined in `src/transport/base-http-host.ts`. The exposed surface is `GET /health` (unauthenticated, returns only `{"status":"ok"}`), `GET /ready` (authenticated readiness with coarse active-model/index/backend check names), plus `GET /sse` + `POST /messages` (SSE) or `POST|GET|DELETE /mcp` (streamable HTTP).

**Mandatory auth.** The server refuses to start in either remote mode without `MCP_AUTH_TOKEN_FILE` or `MCP_AUTH_TOKEN`, and rejects resolved tokens shorter than 32 characters (`src/transport-config.ts:140-205`). `MCP_AUTH_TOKEN_FILE` reads and trims a mounted secret file and takes precedence over the env token. The bearer comparison uses `timingSafeEqual` with a latin1-encoded token buffer so an attacker-controlled `Authorization` header can't be silently re-encoded into an equal-length payload (`src/transport/base-http-host.ts:95-107`, `:451-467`). Repeated bearer failures from the same `remoteAddress` enter a bounded in-memory backoff (`MCP_AUTH_BACKOFF_THRESHOLD`, `MCP_AUTH_BACKOFF_MS`, `MCP_AUTH_BACKOFF_MAX_ENTRIES`) and return `Retry-After`; a successful bearer token clears that address state (`src/transport/base-http-host.ts:350-370`, `:477-538`). Only `/health` is unauthenticated; `/ready` is behind the same auth and origin gates as MCP routes.
...
**Host-header allow-list (DNS-rebinding protection).** Because the Origin check accepts requests with no `Origin` header, a victim's browser tricked via DNS rebinding could otherwise reach the loopback-bound server. When active, the server rejects any non-`/health` request whose `Host` header is missing or not on the allow-list before the auth gate runs (`src/transport/base-http-host.ts` step 3b). The allow-list is derived from `MCP_BIND_ADDR`/`MCP_PORT` (the `host:port` and bare-`host` forms, plus `localhost`/`127.0.0.1`/`[::1]` aliases for loopback binds); `MCP_ALLOWED_HOSTS` adds extra hostnames for reverse-proxy deployments, and `MCP_ALLOWED_HOSTS=*` disables the check when a proxy rewrites `Host` to an unpredictable value. Unlike `MCP_ALLOWED_ORIGINS=*` (rejected, since it would defeat CORS/CSRF), disabling Host validation only forgoes defense-in-depth, so `*` is permitted here.
